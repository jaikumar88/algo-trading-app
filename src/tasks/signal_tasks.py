    ...
    try:
        ...
        if action and symbol and price:
            try:
                ...
                if existing_open:
                    if existing_open.action != action:
                        LOG.warning(
                            f"[OPPOSITE SIGNAL] {action} signal received for {symbol} - IMMEDIATELY closing open {existing_open.action} trade at current price {price}"
                        )
                    else:
                        LOG.info(
                            f"[SAME DIRECTION] {action} signal received for {symbol} - ignoring (already have open {existing_open.action} trade)"
                        )
                ...
                # Enhanced logging for immediate closure behavior
                if res.get('closed'):
                    LOG.info(f"[IMMEDIATE CLOSE] Closed {len(res['closed'])} existing trade(s) for {symbol} at market price {price}")
                if res.get('opened'):
                    LOG.info(f"[NEW TRADE] Opened new {action} trade for {symbol} at {price}")
                else:
                    LOG.info(f"[NO ACTION] Signal ignored - {res.get('message', 'No new trade opened')}")
